...
class MinPriorityQueueTest {
	...
	void setUp() throws Exception {
		pq = new PriorityQueue<>(5, HEAP_TYPE.MIN_HEAP);  
	}
	...
	void testRemoveAt() {
		this.pq.add(2);
		this.pq.add(3);
		assertEquals(2, this.pq.removeAt(0)); 
		assertFalse(this.pq.contains(2));
		assertTrue(this.pq.contains(3));
	}
	...
	void testHeapify() {
		...
		this.pq.add(2);
		this.pq.heapify(0);
		assertEquals(2, this.pq.getElementAt(0));
		assertEquals(4, this.pq.getElementAt(1));
		assertEquals(3, this.pq.getElementAt(2));
	}
